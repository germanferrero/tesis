...
\begin{document}
    ...
    Visualizamos un estudiantado que tenga una primera experiencia con algún lenguaje de programación imperativa, que les permita familiarizarse de manera práctica con los conceptos básicos: tipos de datos, expresiones, precedencia de operadores, árbol de tipado e incluso con la mecánica de los comandos típicos del paradigma: la asignación, la alternativa (\textit{if-else}), y el ciclo (\textit{while/for}). Logrando así, obtener programas imperativos simples y ejecutarlos en una computadora. Esperaríamos de esta experiencia práctica que los estudiantes generen una idea, por imprecisa que fuere, sobre qué es un programa imperativo.

    A continuación, ubicamos una segunda instancia formativa, de carácter formal, que ofrezca a los estudiantes la posibilidad de profundizar su entendimiento de los programas y los comandos del lenguaje como objetos formales.
    En esta segunda instancia los estudiantes razonan sobre estos objetos, ya no en un plano mecánico, operativo, sino en el plano semántico, a partir de las definiciones en semántica de la programación imperativa desarrolladas por Hoare y Dijkstra.
    ...
    La primera pregunta que surge al analizar un programa es tal vez: ¿El programa hace lo que tiene que hacer? Para poder responder esta pregunta primero debemos ser precisos al definir qué debe hacer el programa. A esta tarea la llamamos \textit{especificar} el programa. Escribir la especificación de un programa antes de pensar en implementarlo es (o debiera ser) una práctica ineludible, ya que hacerlo resulta de vital importancia para luego implementar el programa correctamente.
    Al hacerlo, además, logramos separar conceptualmente dos aspectos independientes de un programa: su especificación (qué hace), y su implementación (cómo lo hace).
    Si tenemos un lenguaje lo suficientemente preciso para especificarlo, quien lo utiliza no tiene que preocuparse por cómo está implementado sino solo por comprender su especificación, asumiendo que el programa cumple con ella.

    Hoare y Dijkstra eligieron utilizar fórmulas de lógica de primer orden con sus operadores lógicos ($\land$, $\lor$, $\implies$, $\neg$) y cuantificadores ($\forall$, $\exists$) sobre las variables y constantes del programa para denotar su especificación. Y esta elección se sostiene hasta el día de hoy.

    Precisamente, para especificar un programa utilizamos la Terna de Hoare $\{P\}S\{Q\}$, en la cual $S$ es el programa, $P$ y $Q$ son fórmulas de lógica de primer orden que definen la precondición y poscondición del programa respectivamente, e interpretamos así: 
    \begin{center}
        \textit{Cada vez que ejecutemos $S$ a partir de un estado que satisface $P$ llegaremos a un estado que satisface $Q$.}
    \end{center}

    Un estado, en este contexto es simplemente un mapeo de variables y constantes del programa a sus respectivos valores en un momento dado de la ejecución. La precondición es útil para indicar los requisitos que el programa impone sobre el estado a fin de garantizar la poscondición.

    Ahora que contamos con un formalismo preciso para describir qué debe hacer un programa, queda responder cómo hacemos para garantizar que efectivamente lo hace.

    La respuesta a este interrogante, da lugar a las técnicas de verificación automática de software que, en el caso de Dafny, se basan en las definiciones de Dijkstra acerca de la semántica de los programas.

    \section{Prueba de correctitud}
    Dijkstra introdujo el concepto de \textit{weakest precondition} de un programa respecto de una poscondición $wp.S.Q$ como una fórmula de lógica de primer orden que captura las mínimas condiciones que debe cumplir un estado inicial para que, al ejecutar $S$ a partir de ese estado, se arribe a un estado final en el que se satisface $Q$. A su vez estableció la siguiente equivalencia con la Terna de Hoare que nos dice que un programa cumple con su especificación si y solo si la precondición $P$ implica la \textit{weakest precondition} de $S$ respecto de $Q$.
    \begin{equation}
        \label{eq:terna-de-hoare}
        \{P\}S\{Q\} \equiv P \implies wp.S.Q
    \end{equation}

    De esta manera, si encontramos una demostración de $P \implies wp.S.Q$, por ejemplo, a partir de un conjunto de axiomas que asumimos como tales y un conjunto de reglas de inferencia correctamente utilizadas, habremos garantizado que el programa cumple con su especificación. O en términos de nuestra pregunta original, que el programa hace lo que tiene que hacer.
    ...
    \begin{align*}
        wp.(if\ B\ then\ S\ else\ T).Q \equiv (B \implies wp.S.Q) \land (\lnot B \implies wp.T.Q)
    \end{align*}
    De lo cual se desprende el siguiente teorema análogo al ``teorema 1'' en el trabajo de Dijkstra:
    \begin{align*}
        &(P \land B \implies wp.S.Q) \land (P \land \lnot B \implies wp.T.Q) \\
        &\implies \\
        &(P \implies wp.(if\ B\ then\ S\ else\ T).Q)
    \end{align*}
    ...
    Al definir la $wp$ para la repetición Dijkstra lisa y llanamente se confundió, y nos dejó una definición no acertada y por ende inútil. Lo que sí descubrimos útil más tarde es el siguiente teorema.
    \vspace{1em}
    ...
    \begin{align*}
        \{I \land B\} S \{I\} \land ((I \land \lnot B) \implies Q)
    \end{align*}
    Para los cuales existe una función $t: Estados \to Int$ tal que
    \begin{align*}
        & (i) I \land B \implies t \geqslant 0 \\
        & (ii) \{I \land B \land t=T\} S \{t < T\}
    \end{align*}
    Entonces:
    \begin{align*}
        (P \implies I) \implies \{P\}while\ B\ do\ S\{Q\}
    \end{align*}
    ...
    Contando con un invariante y la existencia de una función de cota que reúnan tales condiciones, el teorema nos dice que: si la precondición $P$ alcanza para establecer el invariante (esto es, $P \implies I$) entonces cada vez que ejecutemos el ciclo a partir de un estado que satisface la precondición $P$, llegaremos a un estado que satisface la poscondición $Q$ (esto es, $\{P\}while\ B\ do\ S\{Q\}$).

    No debemos preocuparnos por no contar con una definición de la $wp$ para la repetición. Dafny resuelve este problema eliminando la repetición del lenguaje intermedio al que traduce los programas que escribimos (y sobre el cuál se realiza el cálculo de \textit{weakest preconditions}), de modo tal que sigue siendo posible obtener la $wp.S.Q$ a partir de la $wp$ de los comandos del lenguaje (de nuevo, los detalles los veremos en el capítulo \ref{ch:pipeline-dafny}). Lo que sí resulta primordial en el proceso de verificación es probar que el invariante se establece al inicio del loop, que se mantiene luego de ejecutar el cuerpo del mismo y que al salir, en conjunción con la negación de la guarda, implica la poscondición deseada.
