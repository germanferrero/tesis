...
method regla_del_fortalecimiento_de_la_precondicion(x: int)
{
    ...
    Aquí hemos utilizado las sentencias \inlinedafny{requires} y \inlinedafny{ensures} para especificar la precondición y poscondición respectivamente del programa $S$. Durante el proceso de verificación del método $regla\_del\_fortalecimiento\_de\_la\_precondicion$, donde se invoca a $S$, Dafny asumirá que $S$ cumple con su especificación, sin inspeccionar su implementación. Esto resulta muy útil en la práctica para partir el programa en programas más pequeños, lograr primero una implementación verificada del método principal, postergando la implementación de las partes.
...
method regla_del_if(x: int)
{
    ...
    Enunciar el invariante y la función de cota incluso antes de implementar el cuerpo del ciclo, nos ayuda a dar sentido a la estrategia resolutiva del mismo, aumenta nuestras posibilidades de implementarlo correctamente y es indispensable al construir una demostración de su correctitud.
    ...
    Recordemos que el cociente y el resto serán números enteros que cumplen:
    ...
    Si pensamos el problema como un proceso iterativo en el que debemos repartir una cantidad $x$ entre $y$ partes iguales, podemos decir que al principio no hemos repartido nada aún, y en cada iteración repartiremos una unidad a cada una de las partes, mientras alcance para todas. Cuando no alcance para todas, habremos terminado, y tendremos un resto $r$ entre 0 e $y$. En todo momento, lo que aún queda por repartir será el resto $r$, la guarda del ciclo será $r \geqslant y$ (aún alcanza para repartir una a cada una) y lo repartido a cada parte será $q$. Entonces al principio tendremos $r=x$ y $q=0$ y en todo momento mantendremos como invariante $x = q * y + r \land 0 \leqslant r$. Notemos que $r$ es mayor a 0 y decrecerá en cada iteración, por lo que $r$ sirve como función de cota que asegura que el ciclo terminará.
    Al final del ciclo, obtendremos el invariante (por haberlo mantenido a cada paso) y la negación de la guarda (por haber terminado). La conjunción de ambas cosas nos dice que: $x = q * y + r \land 0 \leqslant r < y$. Por tanto $q$ y $r$ serán el cociente y resto de la división entera de $x$ por $y$.
