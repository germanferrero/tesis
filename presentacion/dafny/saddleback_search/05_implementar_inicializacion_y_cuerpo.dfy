...
method inicializacion(A: array2<int>, val: int)
  returns (i: int, j:int)
  requires es_una_matriz_ordenada(A) && contiene_el_valor(A, val)
  ensures invariante(A, i, j, val)
{
  return A.Length0 - 1, 0;
}

method cuerpo(A: array2<int>, i: int, j: int, val: int)
  returns (i': int, j':int)
  requires invariante(A, i, j, val)
  requires A[i, j] != val
  ensures invariante(A, i', j', val)
  ensures i' + A.Length1 - j' < i + A.Length1 - j
{
  if val > A[i, j] {
    j' := j + 1;
    i' := i;
